...
const RAYDIUM_CLMM_PROGRAM_ID = new PublicKey('CAMMCzo5YL8w4VFF8KVHrK22GGUQp5VhH5bMKmRoS4tN');
const RAYDIUM_MARKET_PROGRAM_ID = new PublicKey('9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin');
...
// ✅ NEW: Enhanced pool information interface
export interface RaydiumPoolInfo {
  ...
  // ✅ IMPROVED: Actual pool data with version-specific fields
  baseReserve: number;
  quoteReserve: number;
  baseSupply: number;
  quoteSupply: number;
  ...
  poolType?: number;
  targetOrdersOwner?: string;
  withdrawQueueOwner?: string;
  lpVaultOwner?: string;
  rewardInfos?: any[];
  // ✅ NEW: CLMM specific fields
  tickArrays?: string[];
  tickArrayBitmap?: string;
  feeGrowthGlobal?: number;
  feeGrowthGlobalLast?: number;
...
export interface RaydiumPoolState {
  ...
  // add any more fields you may use elsewhere
}
...
export class Raydium {
  ...
  async onNewPool(callback: (poolInfo: RaydiumPoolState) => void): Promise<void> {
  ...
  private parseRaydiumLogs(logs: string[]): RaydiumPoolState | null {
  ...
  private extractRaydiumPoolDataFromLogs(logs: string[]): any {
    const poolData: any = {};
  ...
  private parseRaydiumCLMMLogs(logs: string[]): RaydiumPoolState | null {
    for (const log of logs) {
  ...
  private isKnownProgramId(pubkey: PublicKey): boolean {
    const knownPrograms = [
      RAYDIUM_AMM_PROGRAM_ID,
      RAYDIUM_CLMM_PROGRAM_ID,
  ...
  async createBuyTransaction(tokenMint: PublicKey, amount: number, slippage: number): Promise<Transaction> {
    ...
    if (poolInfo) {
      ...
      const quoteTokenAccount = await getAssociatedTokenAddress(
        new PublicKey(poolInfo.quoteMint), 
  ...
  async getPoolInfo(tokenMint: PublicKey): Promise<RaydiumPoolInfo | null> {
    try {
      ...
      if (ammPools.length > 0) {
        ...
        try {
          ...
          if (poolVersion === 'V2' || poolVersion === 'V3' || poolVersion === 'V4') {
            const v2PlusOffsets = offsets as typeof RAYDIUM_POOL_DATA_OFFSETS.V2;
          ...
          if (poolVersion === 'V3' || poolVersion === 'V4') {
            const v3PlusOffsets = offsets as typeof RAYDIUM_POOL_DATA_OFFSETS.V3;
  ...
  private detectPoolVersion(poolBuffer: Buffer): 'V1' | 'V2' | 'V3' | 'V4' {
    try {
  ...
  private createFallbackPoolInfo(poolData: any, tokenMint: PublicKey): RaydiumPoolInfo {
    return {
      id: poolData.pubkey.toString(),
      baseMint: tokenMint.toString(),
  ...
  private calculateAdvancedPrice(baseReserve: bigint, quoteReserve: bigint): number {
  ...
  private calculateLiquidity(baseReserve: bigint, quoteReserve: bigint): number {
    if (baseReserve === 0n || quoteReserve === 0n) {
      return 0;
    }
  ...
  private readPublicKey(buffer: Buffer, offset: number): PublicKey {
    try {
  ...
  async getAmmId(tokenMint: PublicKey): Promise<PublicKey | null> {
    try {
      const poolInfo = await this.getPoolInfo(tokenMint);
      if (poolInfo) {
        return new PublicKey(poolInfo.id);
      }
      return null;
    } catch (error) {
  ...
  async getAllPoolsForToken(tokenMint: PublicKey): Promise<RaydiumPoolInfo[]> {
    try {
      const pools: RaydiumPoolInfo[] = [];
  ...
  async getBestPoolForToken(tokenMint: PublicKey): Promise<RaydiumPoolInfo | null> {
    try {
